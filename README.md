...
当前基线：展示模式映射 `13/13`、三载体与 A1 迁移 `13/13`、边界渲染 `103/103`，浏览器、静态降级、动效与布局检查 `232/232`。其中包含 7 张 Direct Canvas 的双宏层帧预算、24 张图的“结论铭牌—关键图形”碰撞扫描，以及浅色/深色双表面的填充数据标签对比度检查；GitHub Actions 会在每次推送和 Pull Request 中重新执行这些检查。
