...
- `LOCK` 是结论终态，不等于必须绘制锁环。结论已由颜色、面积、排名或位置唯一表达时使用 `implicit`；只需端点/地址时使用 `micro`；目标仍有歧义或必须连接证据时才使用 `explicit`。
- 重播必须先进入准备态，再在下一帧原子切换到播放态；准备期间不得短暂露出完整终态。整张 SVG 不做位移动画，时间、期限、风险或预测等有序折线可绘制真实路径。
...
- 动画结束后画面适合作为静态截图。
- 重播、暂停、明暗切换和减少动态效果正常。
- 单图同时运行不超过展示模式允许的 2–4 个宏观动画层；有序折线只额外运行少量路径与端点动画。图库默认全静态，只重播用户选中的一张，且不得重新加载 iframe。
- 连续重播不得出现元素由完成态跳回隐藏态的 `1 → 0 → 1` 显隐闪回。
