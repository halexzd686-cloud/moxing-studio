...
def main() -> None:
    ...
    for line in REFERENCE.read_text(encoding="utf-8").splitlines():
        match = re.match(r"\| (C\d+) \| [^|]+ \| ([ABC]) \| ([a-z]+) \| (implicit|micro|explicit) \| [^|]+ \| (low|medium|high) \| ([ABC]) \|", line)
        if match:
            documented_rows.append(match.groups())
    contract_rows = [(item["id"], item["mode"], item["motion"], item["lockMode"], item["risk"], item["batch"]) for item in charts]
    ...
    checks = {
        ...
        "valid lock intensities": all(item.get("lockMode") in data["lockModes"] for item in charts),
