...
def main() -> None:
  ...
  <div class="frame"><iframe title="{meta['name']}" loading="lazy" src="{filename}?motion=brief&autoplay=off&build=precision-v2.1"></iframe></div>
</article>"""
        )
    html = f"""<!DOCTYPE html>
...
function fit(){{document.querySelectorAll('.frame').forEach(box=>{{const frame=box.querySelector('iframe'),scale=box.clientWidth/1280;if(CSS.supports('zoom','1')){{frame.style.zoom=String(scale);frame.style.transform='none'}}else{{frame.style.zoom='1';frame.style.transform=`scale(${{scale}})`}}}})}}
function update(frame){{frame.src=frame.src.split('?')[0]+`?motion=${{motion.value}}&theme=${{dark?'dark':'light'}}&autoplay=off&build=precision-v2.1`}}
function getApi(frame){{try{{return frame.contentWindow?.Moxing}}catch{{return null}}}}
function replay(frame){{frames.forEach(item=>{{if(item!==frame)getApi(item)?.settle()}});const api=getApi(frame);if(api)api.replay();else{{frame.addEventListener('load',()=>getApi(frame)?.replay(),{{once:true}});update(frame)}}}}
...
frames.forEach(frame=>frame.addEventListener('load',()=>getApi(frame)?.settle()));addEventListener('resize',fit,{{passive:true}});fit();
</script></body></html>"""
