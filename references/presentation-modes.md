...
Sequence: `DATA_FIELD -> RELATIONSHIP -> [TARGET_LOCK]`. The relationship cue carries the chart's own geometry; it must not manufacture a detached evidence object.
...
Sequence: `DATA_FIELD -> RELATIONSHIP -> LOCAL_EVIDENCE -> [TARGET_LOCK]`.
...
Sequence: `DATA_FIELD -> EVIDENCE_BAY -> TERMINAL -> [TARGET_LOCK]`.

## Lock intensity

- `implicit`: the chart's signal color, area, rank, or position already identifies the conclusion; add no ornament or late lock animation.
- `micro`: retain only a compact socket, endpoint tick, range bracket, or address. Remove focus corners, rings, and long leaders.
- `explicit`: use a focus corner/ring and evidence relationship only when the target cannot be inferred safely from primary geometry.
...
| C11 | Sector Lock | A | assemble | implicit | dominant sector color/area is primary | low | A |
...
| C20 | Sensitivity Matrix | A | scan | implicit | selected cell color remains primary | medium | A |
...
- `assemble`: establish a datum, then seat a complete group of marks with element-level cues.
...
Repeated marks may be authored with detailed `ALIGN / DOCK / ROUTE / LOCK` semantics. Production playback compiles those semantics into a single forward cue timeline; it does not animate every bar, cell, candle, label, or glyph as a macro group.

## Migration batches

1. **I0 / carriers**: replace binary `precision | legacy` branching with explicit `direct | embedded | interface` carriers while preserving final SVG geometry and static fallbacks.
2. **A / Direct Canvas**: C1, C2, C4, C5, C7, C9, C10, C11, C12, C20.
3. **B / Embedded Evidence**: C13, C14, C16, C17, C18, C19, C21, C23, C24.
4. **C / Evidence Interface**: C3, C6, C8, C15, C22.
...
- B mode contains no split body; its evidence capsule stays inside verified natural whitespace.
- C mode reserves the full 220px bay and 28px safety gap; evidence ID, terminal, and target lock agree.
- Collision checks run in light and dark surfaces at 1280px and in the narrow Gallery viewport.
- Replay uses one forward-only state machine: `settle -> is-resetting -> is-playing -> is-complete`.
- `is-resetting` is one layout boundary only; no reverse animation, second reveal pass, or completed path is exposed during reset.
- Gallery remains static until requested and reuses the selected iframe without reloading it.
